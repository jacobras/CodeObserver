...
private val DEMO_DETEKT_METRICS = mutableListOf(
    DetektMetricDto(
        id = ReportId(1),
        projectId = ProjectId("myApp"),
        gitHash = GitHash("a1b2c3d"),
        gitDate = now.minus(99.days),
        findings = 250,
        smellsPer1000 = 12
    ),
    DetektMetricDto(
    ...
    DetektMetricDto(
        ...
        gitDate = now.minus(1.days),
        findings = 421,
        smellsPer1000 = 21
    ),
)

internal class DemoDetektReportDataSource : DetektReportDataSource {
    ...
    override suspend fun fetchMetrics(projectId: ProjectId): Result<List<DetektMetricDto>, NetworkError> =
        Ok(DEMO_DETEKT_METRICS.filter { it.projectId == projectId })

    override suspend fun delete(reportId: ReportId): Result<Unit, NetworkError> {
        DEMO_DETEKT_METRICS.removeAll { it.id == reportId }
        return Ok(Unit)
    }
}